...
def loadFile():
    ...
    for item in treeList.get_children():
        treeList.delete(item)
    filetypes = (("Picle files", "*.pkl"), ("All files", "*.*"))
    global listOfVehicles
...
button5 = Button(
    window, width=10, text="Sort Mileage", bg="#99CCAA", command=sortMileage
)
button5.grid(row=1, column=5)


button7 = Button(
    window, width=10, text="Create Pickle", bg="#99CCAA", command=createPickle
)
...
style = Style()
style.theme_use("alt")
style.configure("Treeview", background="silver", foreground="green")
style.map("Treeview", background=[("selected", "#FF1800")])
treeList = Treeview(
    columns=("Owner", "Vendor", "Model", "Type", "Reg Number", "Eng Number", "Mileage"),
    show="headings",
)
